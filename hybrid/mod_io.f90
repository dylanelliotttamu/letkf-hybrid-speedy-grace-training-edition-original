module mod_io
   ...
   contains 
     ...
     subroutine read_netcdf_4d_old(varname,filename,var)
     ...
     subroutine read_netcdf_4d(varname,filename,var,time_index)
        character(len=*), intent(in) :: varname
        character(len=*), intent(in) :: filename
        ...
        integer :: start4d(numofdims)
        integer :: count4d(numofdims)
        ...
        start4d = [1,1,1,start_time_index]
        count4d = [dim_length(1),dim_length(2),dim_length(3),dim_length(4)]

        call nc_check(nf90_get_var(ncid,varid,var,start=start4d,count=count4d))
    ...
    subroutine read_era_data_parallel(filename,model_parameters,mpi_res,grid,era_data,start_time_arg,stride_arg)
      ...
      era_var_names(1)%str = 't'
      era_var_names(2)%str = 'u'
      era_var_names(3)%str = 'v'
      era_var_names(4)%str = 'q'
      era_var_names(5)%str = 'ps'
   ...
   subroutine read_full_file_4d(grid4d,grid2d,timestep,filename)
    ...
    subroutine read_trained_res(reservoir,model_parameters,grid)
      ...
      call read_netcdf_1d_dp_opened('res_state01',ncid,reservoir%saved_state)
