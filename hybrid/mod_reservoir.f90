...
subroutine initialize_model_parameters(model_parameters,processor,num_of_procs)
   ...
   write_training_weights = .False.!TRUEE 6/21

   model_parameters%num_predictions = 730!5!0
   ...
   model_parameters%discardlength = 4*10! timesteps/day * days !24*10!7
   model_parameters%traininglength = 12!8760*29!365*24 = 8760 !28 !29 HERE  !14000*6 !
   ! prediction length is number of hours so 240 = 24*10
   model_parameters%predictionlength = 24*10! for 2011 24*2 worked 24*4 works !2*8760!24!*100!8760*3!1 + 24*5!8760*30 + 24*5!504!8760*11 + 24*5 !504!0
   model_parameters%synclength = 12!24*4!24*4!24*14*2 !+ 180*24
   ...
   model_parameters%train_on_era5 = .False.
...
subroutine get_training_data_era5(reservoir,model_parameters,grid,loop_index)
   ...
   call initialize_calendar(calendar,2011,1,1,0)
...
subroutine get_prediction_data(reservoir,model_parameters,grid,start_index,length)
   ...
   !Read data in stride and whats only needed for this loop of training
   call read_letkf_analysis(reservoir,grid,model_parameters,calendar%startyear,calendar%currentyear,era_data)
   print *, 'dylan start_index == ', start_index
  ...
  if(allocated(reservoir%predictiondata)) then
   ...
   ! dylan CHanging how allocate is done below
  allocate(reservoir%predictiondata(reservoir%reservoir_numinputs,length/model_parameters%timestep))
  !allocate(reservoir%predictiondata(reservoir%reservoir_numinputs,696) ! 32 +1 now? this is apparently not doing anything.  
  

  ! HERES WHERE ITS shaped, try changing this
  !!
  ! I have 1, here, try at timestep 28.. for starting from (7*4) days after +1
  ! for index starting at 1 right. No, i think I changed start_index so we are
  ! good

  !reservoir%predictiondata(grid%atmo3d_start:grid%atmo3d_end,:) = reshape(era_data%eravariables(:,:,:,:,((7*4)+1):end_time_memory_index),[grid%atmo3d_end,length/model_parameters%timestep])
  ! original below
  ...
  if(reservoir%logp_bool) then 
    !reservoir%predictiondata(grid%logp_start:grid%logp_end,:) = reshape(era_data%era_logp(:,:,((7*4)+1):end_time_memory_index),[reservoir%logp_size_input,length/model_parameters%timestep])
...
subroutine initialize_prediction(reservoir,model_parameters,grid)
   ...
   ! dylan here!!!
   call get_prediction_data(reservoir,model_parameters,grid,6,un_noisy_sync)
   ! original below 
   ...
   print *, "here past get_prediction_data call 1"
...
subroutine start_prediction(reservoir,model_parameters,grid,prediction_number)
   ...
   print *, 'dylan here before  call get_prediction_data ' 
  
   ! start from index 1
   call get_prediction_data(reservoir,model_parameters,grid,model_parameters%prediction_markers(prediction_number)+6,model_parameters%synclength+model_parameters%predictionlength+10)!+100)
   ! start from 7 days in ya 
   !call get_prediction_data(reservoir,model_parameters,grid,model_parameters%prediction_markers(prediction_number)+6,model_parameters%synclength+100) 
   
   !call get_prediction_data(reservoir,model_parameters,grid,(24*7) * model_parameters%timestep,model_parameters%synclength+model_parameters%predictionlength+10)!+100)
   ! so the 4th dimension here (start_index..) is where to start the first prediction from, so 7
   ! days in shoudl be (7*24) * 6 because later in start_time_mem_index it
   ! divides start_index by 6 


   print *, 'dylan here call get_prediction_data DONE'
   ! below is for starting from 1981, making predictions at some time after that 
   !call get_prediction_data(reservoir,model_parameters,grid,model_parameters%traininglength+model_parameters%prediction_markers(prediction_number)+2*model_parameters%timestep,model_parameters%synclength+model_parameters%predictionlength+10)!+100)

   !call get_prediction_data(reservoir,model_parameters,grid,model_parameters%traininglength+model_parameters%prediction_markers(prediction_number)+2*model_parameters%timestep,model_parameters%synclength+100)!+100)
...
subroutine write_trained_res(reservoir,model_parameters,grid)
  ...
  file_path='/scratch/user/dylanelliott/ML_SPEEDY_WEIGHTS_ERA5_weights_6hr_timestep/'
...
subroutine write_controller_file(model_parameters)
   ...
   file_path = '/scratch/user/dylanelliott/ML_SPEEDY_WEIGHTS_ERA5_weights_6hr_timestep/'//trim(model_parameters%trial_name)//'_controller_file.txt'
