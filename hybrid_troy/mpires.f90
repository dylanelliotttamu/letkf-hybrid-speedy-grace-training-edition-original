   ...
   contains
     ...
     subroutine write_truth_data(res,timestep)
        ...
        if(.not.(mpi_res%is_root)) then
           do i=1, res%model_parameters%num_of_regions_on_proc
              do j=1, res%model_parameters%num_vert_levels
                 ...
                 if(res%reservoir(i,j)%assigned_region == 954) print *,'reservoir%prediction used to make first truth data',res%reservoir(i,j)%predictiondata(:,res%model_parameters%synclength/res%model_parameters%timestep+timestep)
                 call unstandardize_state_vec_input(res%reservoir(i,j),res%grid(i,j),copy_truth_vec) 
       ...
       if(mpi_res%is_root) then
           ...
           call get_current_time_delta_hour(calendar,res%model_parameters%traininglength+res%model_parameters%synclength+res%model_parameters%prediction_markers(res%model_parameters%current_trial_number))
           print *, 'res%model_parameters%traininglength,res%model_parameters%synclength,res%model_parameters%prediction_markers(res%model_parameters%current_trial_number),timestep',res%model_parameters%traininglength,res%model_parameters%synclength,res%model_parameters%prediction_markers(res%model_parameters%current_trial_number),timestep
           write(year,'(I4.4)') calendar%currentyear
           write(month,'(I2.2)') calendar%currentmonth
           write(day,'(I2.2)') calendar%currentday
           write(hour,'(I2.2)') calendar%currenthour
     ...
     subroutine write_truth_data_ensemble(res,timestep,ensemble_num)
       !subroutine to write the true data to a file
       ...
       !input variables
       integer, intent(in)            :: timestep, ensemble_num
       ...
       character(len=2)  :: day
       character(len=2)  :: hour
       character(len=3)  :: ens_num
       character(len=:), allocatable :: date_file
       character(len=:), allocatable :: truth_out_file_name
       character(len=:), allocatable :: file_path
       
       !real, dimension(ix*il,kx) ::ugr_h, vgr_h, tgr_h, qgr_h
       !real :: psgr_h(ix*il), rrgr4(ix*il)
       
       real(4), dimension(ix*il,kx) :: ugr_h, vgr_h, tgr_h, qgr_h
       real(4), dimension(ix*il) :: psgr_h(ix*il), rrgr4(ix*il)
       ...
       integer :: irec, k
        ...
        if(mpi_res%is_root) then
           ...
           do i=1, res%model_parameters%num_of_regions_on_proc
              do j=1, res%model_parameters%num_vert_levels
                 ...
                 !print *, 'shape(res%reservoir(i,j)%predictiondata)',shape(res%reservoir(i,j)%predictiondata)
                 !print *, 'res%model_parameters%synclength/res%model_parameters%timestep+timestep',res%model_parameters%synclength/res%model_parameters%timestep+timestep
                 copy_truth_vec = res%reservoir(i,j)%trainingdata(1:res%reservoir(i,j)%reservoir_numinputs-res%reservoir(i,j)%sst_size_input-res%reservoir(i,j)%tisr_size_input,timestep)
                 call unstandardize_state_vec_input(res%reservoir(i,j),res%grid(i,j),copy_truth_vec)
        ...
        if(.not.(mpi_res%is_root)) then
           do i=1, res%model_parameters%num_of_regions_on_proc
              do j=1, res%model_parameters%num_vert_levels
                 local_domain_size = size(res%reservoir(i,j)%trainingdata(1:res%reservoir(i,j)%reservoir_numinputs-res%reservoir(i,j)%tisr_size_input-res%reservoir(i,j)%sst_size_input,timestep))

                 allocate(copy_truth_vec(res%reservoir(i,j)%reservoir_numinputs-res%reservoir(i,j)%tisr_size_input-res%reservoir(i,j)%sst_size_input))

                 copy_truth_vec = res%reservoir(i,j)%trainingdata(1:res%reservoir(i,j)%reservoir_numinputs-res%reservoir(i,j)%tisr_size_input-res%reservoir(i,j)%sst_size_input,timestep)
                 if(res%reservoir(i,j)%assigned_region == 954) print *,'reservoir%prediction used to make first truth data',res%reservoir(i,j)%trainingdata(:,timestep)
                 call unstandardize_state_vec_input(res%reservoir(i,j),res%grid(i,j),copy_truth_vec) 
       ...
       if(mpi_res%is_root) then
           ...
           write(ens_num,'(I3.3)') ensemble_num

           date_file = month//'_'//day//'_'//year//'_'//hour
           truth_out_file_name = 'climo_ensemble_data_member'//ens_num//'.grd'!file_path//truth_out_root//res%model_parameters%trial_name//trial_word//date_file//file_end
           ! wholegrid4d(variable,lat,long,sigma_level) troy thinks this is the
           ! format and there is no time dimension
           ugr_h = reshape(wholegrid4d(2,:,:,:),[xgrid*ygrid,zgrid])
           vgr_h = reshape(wholegrid4d(3,:,:,:),[xgrid*ygrid,zgrid])
           tgr_h = reshape(wholegrid4d(1,:,:,:),[xgrid*ygrid,zgrid])
           qgr_h = reshape(wholegrid4d(4,:,:,:),[xgrid*ygrid,zgrid])
           psgr_h = reshape(wholegrid2d(:,:),[xgrid*ygrid])
           
           ! DYLAN 
           !print *, 'ugr_h before halving timesteps', size(ugr_h)
           
           qgr_h = qgr_h*1.0d-3 ! kg/kg
           psgr_h = 100000.0*exp(psgr_h)! hPa changed to Pa by adding 2 zeros
           ! here
           rrgr4 = 0.0_dp
           
           ! DYLAN 
           ! ugr_h = reshape(wholegrid4d(2,:,:,:18432),[xgrid*ygrid,zgrid])
           !ugr_h = reshape(wholegrid4d(2,:18432,:,:),[xgrid*ygrid,zgrid])

           ! code failed here 11/29/23 1:55pm because 18432 is placed in
           ! incorrect dimension
           !print *, 'ugr_h after halving timesteps', size(ugr_h)   

           open (99,file=truth_out_file_name,form='unformatted',access='direct',&
               & recl=4*ix*il) ! original recl=4*ix*il

           print *, '4*ix*il',4*ix*il
           print *, 'kx',kx
           print *, 'ngp = ',ngp 
           print *, 'ix',ix
           print *, 'il',il
           print *, 'Size of ugr_h:', size(ugr_h)
           !Code should be fully working up to this point after it fails related
           !to the writing of the grd files
           irec=1
           print *,'made it to ugr_h'
           do k=1,kx!kx,1,-1!k=1,kx!kx,1,-1 ! -1
              write (99,rec=irec) (ugr_h(j,k),j=1,ngp)
              irec=irec+1
           end do
           print *,'made it to vgr_h'

           do k=1,kx!kx,1,-1!k=1,kx!kx,1,-1
              write (99,rec=irec) (vgr_h(j,k),j=1,ngp)
              irec=irec+1
           end do
           print *,'made it to tgr_h'

           do k=1,kx!kx,1,-1!k=1,kx!kx,1,-1
              write (99,rec=irec) (tgr_h(j,k),j=1,ngp)
              irec=irec+1
           end do
           print *,'made it to qgr_h'

           do k=1,kx!kx,1,-1!k=1,kx!kx,1,-1
              write (99,rec=irec) (qgr_h(j,k),j=1,ngp)
              irec=irec+1
           end do
           print *,'made it to psgr_h'
           ! Converting to Pa ,, not hPa 
           write (99,rec=irec) (psgr_h(j),j=1,ngp)
           irec=irec+1
           print *,'made it to rrgr4'

           write (99,rec=irec) (rrgr4(j),j=1,ngp)
           close (99)
           print *, 'Finished writing .grd file for ens_num =',ens_num
       endif
       print *,'Finished all..'
       return
     end subroutine
     ...
     subroutine run_model(model_parameters,timestep,grid4d,grid2d,sst_grid,speedy_grid4d,speedy_grid2d)
       ...
       if(.not. allocated(internal_state_vector%variables3d)) then
        ...
        internal_state_vector%variables3d = copy!(:,:,48:1:-1,8:1:-1)
        internal_state_vector%logp = grid2d!(:,48:1:-1)
        ...
        !speedy_grid4d = speedy_grid4d(:,:,48:1:-1,8:1:-1)
        !speedy_grid2d = speedy_grid2d(:,48:1:-1)
