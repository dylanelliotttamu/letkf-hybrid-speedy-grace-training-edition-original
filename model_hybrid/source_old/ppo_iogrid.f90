subroutine iogrid(imode)
    ...
    use mod_physvar, only : ug1_ic, vg1_ic, tg1_ic, qg1_ic, phig1_ic, pslg1_ic, ugr_h, vgr_h, tgr_h, qgr_h, psgr_h
    use mod_dynvar
    use mod_dyncon1
    use mod_date
    use mod_tsteps
    ...
    if (imode.eq.1) then
        ...
        if (imode.eq.2) then ! p-level output
            ...
            do k=1,kx
                ...
                do j=1,ngp
                  rrgr1(j) = save2d_d2(j,1)& ! g/m^2/s   !NOTE IM pretty sure this is wrong off by a factor of 3.6*4.0*6.0
