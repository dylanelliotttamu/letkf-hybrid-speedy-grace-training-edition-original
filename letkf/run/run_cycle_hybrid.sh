...
then
    # Mixed precision SPEEDY
    cp $SPEEDY/model_hybrid/source/makefile .
    cp $SPEEDY/model_hybrid/source/*.h .
    cp $SPEEDY/model_hybrid/source/*.f90 .
    cp $SPEEDY/model_hybrid/source/*.s .
...
do
    ...
    do
        ...
        sh ensfcst_hybrid.sh $SPEEDY $OUTPUT $YYYY$MM$DD$HH $TY$TM$TD$TH $MEM $N &
